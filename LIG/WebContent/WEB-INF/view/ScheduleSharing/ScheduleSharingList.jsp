...
<body>
	...
	<section id="sideMenu" class="section section-padded">
		...
		<div class="row text-center title" style="margin-top: -50px;">
							...
							<div style="margin-top: 20px; margin-bottom: -20px">
								<c:if test="${sessionScope.mem_id == 'admin'}">
									<form name="del" method="get"
										...
										<input type="submit" value="삭제"  class="a btn-orange-fill"/>
			...
			<ul class="pagination">
				...
				<c:if test="${pageCount > 0}">
					<c:if test="${startPage > pageBlock}">
						<a
							href="<c:url value='/ScheduleSharing/PreScheduleSharingList.do?pageNum=${startPage - pageBlock }'/>">[이전]&ensp;</a>
					</c:if>

					<c:forEach var="i" begin="${startPage}" end="${endPage}">
						<a
							href="<c:url value='/ScheduleSharing/PreScheduleSharingList.do?pageNum=${i}'/>">[${i}]</a>
					</c:forEach>

					<c:if test="${endPage < pageCount}">
						<a
							href="<c:url value='/ScheduleSharing/PreScheduleSharingList.do?pageNum=${startPage + pageBlock }'/>">&ensp;[다음]</a>
					</c:if>
				</c:if>
			</ul>
		</div>
