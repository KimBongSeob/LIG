...
<body>
	...
	<h3><caption>Q&A 답변작성</caption></h3>
	</div>
	
	<form action="/LIG/QuestionBoard/qboardreplywritepro.do?num=${num} " method="post" id="frm" name="frm" onsubmit="return subjectCheck();">
		
		<br />
		<!-- 원글에 대한 정보, 글번호,ref,re_step,re_level -->
		<input type="hidden" name="board_num" value="${num}"/>
		<input type="hidden" name="ref" value="${boardref}"/> 
		<input type="hidden" name="re_step" value="${boardrestep}"/> 
		<input type="hidden" name="re_level" value="${boardrelevel}"/> 
		...
		<input type="button" class="btn btn-primary" id="golist" value="목록으로" onclick="location.href='qboardlist.do'" />
		</div>
	</form>
