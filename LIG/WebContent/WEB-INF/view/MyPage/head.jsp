...
$(function(){
    $(document).on("click","#delbutton",function(){ 
        var list = new Array();
    	$(".arearesultUl > .arearesultLi").each(function(i) {
            
            if($(this).children().eq(0).children().prop("checked")){
                 list.push($(this).children().eq(1).attr("value"));
            }

        });
    	
    	var url="<%=cp%>/mypage/scheduleSharing/delitem.do";
		$.ajax({
			type : "post",
			url : url,
			data : "group_id="+list,
			...
			success : function(args) {
				if(args.count > 0){
					alert("일정공유 게시물을 "+args.count+"개 삭제하셨습니다.");
					...
					for(var i=0;i<itemList.length;i++){
						if(itemList[i].firstimage != ""){
							$("#arearesultUl").append("<li class='arearesultLi'><span style=\"float:left; width:50px; -webkit-transform: scale(1.5); margin-top:20px; margin-left:-15px;\"><input type=\"checkbox\" id=\"delcheck\" name=\"delcheck\"></span>"
									+"<input type=\"hidden\" id=\"group_id\" name=\"group_id\" value=\""+itemList[i].group_id+"\"><div class='service' style=\"background-image: url("+itemList[i].firstimage+");\">"
									+"<div class='icon-holder'><a href='' alt=''><p>"+itemList[i].start_date+" ~ "+itemList[i].end_date
									+"<br>"+itemList[i].subject+"</p></a></div>"
									+"<a href='' alt='' style=\"text-decoration:none; text-shadow: rgb(0, 0, 0) 1px 1px 5px, rgb(206, 170, 156) 1px 1px 5px;\"><p class='description overflow' style=\"color: #f5f5f5; background-color: rgba(132, 119, 112, 0.53);\">"+itemList[i].share_content+"</p></a></div></li>");
						}else if(itemList[i].firstimage == ""){
							$("#arearesultUl").append("<li class='arearesultLi'><span style=\"float:left; width:50px; -webkit-transform: scale(1.5); margin-top:20px; margin-left:-15px;\"><input type=\"checkbox\" id=\"delcheck\" name=\"delcheck\"></span>"
									+"<input type=\"hidden\" id=\"group_id\" name=\"group_id\" value=\""+itemList[i].group_id+"\"><div class='service' style=\"background-image: url(/LIG/img/unknown2.png);\">"
									+"<div class='icon-holder'><a href='' alt=''><p>"+itemList[i].start_date+" ~ "+itemList[i].end_date
									+"<br>"+itemList[i].subject+"</p></a></div>"
									+"<a href='' alt='' style=\"text-decoration:none; text-shadow: rgb(0, 0, 0) 1px 1px 5px, rgb(206, 170, 156) 1px 1px 5px;\"><p class='description overflow' style=\"color: #f5f5f5; background-color: rgba(132, 119, 112, 0.53);\">"+itemList[i].share_content+"</p></a></div></li>");
						}	
					...
					if(pageCount > 0){
						if (startPage > pageBlock) {
							var sum = startPage - pageBlock;
							$(".pagination").append("<a href=\"<c:url value='/mypage/scheduleSharing/selectList.do?pageNum=1'/>\">[<<]</a>");
							$(".pagination").append("<a href=\"<c:url value='/mypage/scheduleSharing/selectList.do?pageNum="+sum+"'/>\">[이전]&ensp;</a>");
						}

						for(var i = startPage;i<=endPage;i++) {
							$(".pagination").append("<a href=\"<c:url value='/mypage/scheduleSharing/selectList.do?pageNum="+i+"'/>\">["+i+"]</a>");
						}

						if (endPage < pageCount) {
							var sum = startPage + pageBlock ;
							$(".pagination").append("<a href=\"<c:url value='/mypage/scheduleSharing/selectList.do?pageNum="+sum+"'/>\">[다음]&ensp;</a>");
							$(".pagination").append("<a href=\"<c:url value='/mypage/scheduleSharing/selectList.do?pageNum=1'/>\">[>>]</a>");
						}
					}
				}
...
$(function(){//전체선택 체크박스 클릭
	$("#all_delcheck").click(function(){ //만약 전체 선택 체크박스가 체크된상태일경우
		if($("#all_delcheck").prop("checked")) { //해당화면에 전체 checkbox들을 체크해준다
